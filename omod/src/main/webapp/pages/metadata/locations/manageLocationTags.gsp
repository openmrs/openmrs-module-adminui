	 	...
	 	<tr>
			...
			<td valign="top">
                <form id="adminui-restore-form-${it.id}" method="POST">
                    ...
                    <% if(!it.retired) { %>
                    <i class="icon-remove delete-action" title="${ ui.message("general.retire") }" onclick="adminui_retireLocationTag(${it.id}, '${ui.encodeJavaScriptAttribute(it.name)}')"></i>
				    <% } else { %>
                    <i class="icon-reply edit-action" title="${ ui.message("general.restore") }" onclick="adminui_restoreLocationTag(${it.id})"></i>
				    <% } %>
                    <i class="icon-trash delete-action right" title="${ ui.message("general.purge") }" onclick="adminui_purgeLocationTag(${it.id}, '${ui.encodeJavaScriptAttribute(it.name)}')"></i>
